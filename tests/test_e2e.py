...
def test_tiny_transcribes_the_sample(sample_wav, tmp_path):
    outs = run(_job(sample_wav, tmp_path), Engine("tiny", CPU), CPU)
...
def test_resume_with_real_engine(sample_wav, tmp_path):
    ...
    clean = run(job, engine, CPU)[1].read_text(encoding="utf-8")
    ...
    with Checkpoint(checkpoint_db()) as ck:
        ...
        ck.forget(key)
        ck.open_job(key, sample_wav)
        ck.append(key, kept)
    resumed = run(_job(sample_wav, tmp_path / "b"), engine, CPU)[1].read_text(encoding="utf-8")
